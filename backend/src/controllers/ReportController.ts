...
export const getReportSummary = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    // 2. Room Occupancy
    const totalRooms = await Room.countDocuments({ houseId: req.houseId });
    const rooms = await Room.find({ houseId: req.houseId });
    ...
    const userMealsMap: { [userId: string]: number } = {};
...
export const exportReportPDF = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const members = await User.find({ houseId: req.houseId }).select("name username role");
